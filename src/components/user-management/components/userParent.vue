    ...
    <el-col class="right_table" style="width: 1166px;margin-left: 8px;">
        ...
        <el-row class="table-wrap">
            <el-table
                ...
                <el-table-column prop="unit_name" align="center" label="所属学校"> </el-table-column>
                <el-table-column prop="user_account" align="center" label="账号"> </el-table-column>
                <el-table-column prop="user_name" align="center" label="学生姓名"> </el-table-column>
...
<script lang="ts">
    import {Parent,PageQuery} from "../user-management.def";
    ...
    @Component
    export default class Parents extends XtVue {
        ...
        public getParentData(): void{
            ...
            let obj = {};
            if (this.roleRank == '0') {
                obj = {page_size: this.pageQuery.page_size, page_num: this.pageQuery.page_num, user_type: '7', fk_unit_id: ''};
            } else {
                obj = {page_size: this.pageQuery.page_size, page_num: this.pageQuery.page_num, user_type: '7', fk_unit_id: this.pageQuery.fk_unit_id};
            }
            this.loading = true;
            UserManageCnt.cloud.ces(obj).then((response: AxiosResponse)=>{
                if(response.data.status != 200){
                    ...
                    this.loading = false;
                    this.parentData = response.data.data.content;
                    this.pageSize = response.data.data.size;
                    this.total = response.data.data.totalElements;
                }
            })
            // UserManageCnt.cloud.getAllUserData(this.pageQuery).then((response: AxiosResponse)=>{
            //     if(response.data.status != 200){
        ...
        insureStatus(): void{
            ...
            this.loading = true;
            UserManageCnt.cloud.mulitpleEditUserStatus(ids,this.batchStatus).then((response:AxiosResponse)=>{
                if(response.data.status != 200){
                    this.loading = false;
                    this.any().$notify.error({ title: '提示', message: response.data.message });
                }else{
