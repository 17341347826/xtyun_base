        ...
        <el-row>
            <template>
                <el-table :data="disciplineList"
                    ...
                    <el-table-column header-align="center" prop="discipline_name" label="学科名称"></el-table-column>
                    <el-table-column header-align="center" prop="discipline_code" label="学科代码"></el-table-column>
            ...
            <el-row class="dialog_center">
                <el-row>
                    <el-tooltip class="item" effect="dark" content="学科名称" placement="right">
                        <el-input v-model="disciplineAddList.discipline_name" placeholder="学科名称" clearable style="width: 315px"
                ...
                <el-row>
                    <el-tooltip class="item" effect="dark" content="学科代码" placement="right">
                        <el-input v-model="disciplineAddList.discipline_code" placeholder="学科代码" clearable style="width: 315px"
                                  maxlength="10"></el-input>
                    </el-tooltip>
                    <span class="required">*</span>
                </el-row>
...
<script lang="ts">
    ...
    import {ChangeStatus, PageQuery, DisciplineAddList,} from "../department.def";
    ...
    export default class Subject extends XtVue {
        ...
        // 表格数据
        private disciplineList: any = [];
        // 新增弹框
        private addVisible: boolean = false;
        // 新增的数据
        private disciplineAddList: DisciplineAddList = new DisciplineAddList();
        ...
        getInfo(): void {
            ...
            this.loading = true;
            DepartmentCnt.cloud.list_page_all_discipline_info(this.pageQuery).then((response: AxiosResponse) => {
                if (response.data.status === '200') {
                    this.loading = false;
                    this.pageTotal = response.data.data.totalElements;
                    this.disciplineList = response.data.data.content;
                } else {
                    this.loading = false;
        ...
        onAddVisible(state: number, row: any = ''): void {
            if (state === 0) { // 点击新增
                if (row) {
                    this.addTitle = '修改';
                    this.disciplineAddList.id = row.id;
                    this.disciplineAddList.discipline_code = row.discipline_code;
                    this.disciplineAddList.discipline_name = row.discipline_name;
                    this.disciplineAddList.status = row.status;
                } else {
                    this.addTitle = '新增';
                    this.disciplineAddList = new DisciplineAddList();
                }
                this.addVisible = true;
            } else if (state === 1) { // 取消
                this.addVisible = false;
            } else if (state === 2) { // 确定
                if (this.disciplineAddList.discipline_name && this.disciplineAddList.discipline_code) {
                    this.disciplineAddList.fk_unit_id = this.unitID;
                    this.loading = true;
                    DepartmentCnt.cloud.save_or_modify_discipline_info(this.disciplineAddList).then((response: AxiosResponse) => {
                        if (response.data.status === '200') {
                            this.loading = false;
                            this.getInfo();
                            this.disciplineAddList = new DisciplineAddList();
        ...
        onChangeStatus(obj: any): void {
            ...
            this.loading = true;
            DepartmentCnt.cloud.modify_discipline_info_status_by_id(this.changeStatus).then((response: AxiosResponse) => {
                if (response.data.status === '200') {
                    this.loading = false;
                    this.getInfo();
                } else {
        ...
        onDelConfirm(): void { // 确认删除
            this.loading = true;
            DepartmentCnt.cloud.remove_discipline_info_by_id(this.delId).then((response: AxiosResponse) => {
                if (response.data.status === '200') {
                    this.loading = false;
                    this.getInfo();
